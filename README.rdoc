...
MVC port of Nico Ritsche's multiple file S3 uploader Rails example, which can be found at https://github.com/ncri/s3_uploader_example
...
2. Fill in the AWS S3 config variables within S3Uploader\S3PrepTask\App.config and S3Uploader\S3Uploader\Web.config:
	...
	<add key="AWSBucket" value="" />
	<add key="AWSUploaderPath" value="uploader/"/>

4. Set your server host in S3Uploader/S3Uploader/Scripts/s3_uploader/s3_uploader.js:
    var host = 'http://yourdomain.com';

5. Deploy uploader javascript and form to s3 (run in command prompty)
    S3Uploader\S3PrepTask\bin\{debug or release}\S3PrepTask.exe

6. Run or deploy the web app :)
